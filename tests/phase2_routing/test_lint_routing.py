...
class TestLintRouting:
    ...
    def test_lint_fail_first_retry_routes_to_debugger(self):
        """Test lint failure on first retry routes to debugger."""
        state = create_initial_state("/tmp/test")
        state["lint_output"] = StageOutput(
            success=False,
    ...
    def test_lint_fail_second_retry_routes_to_debugger(self):
        """Test lint failure on second retry still routes to debugger."""
        state = create_initial_state("/tmp/test")
        state["lint_output"] = StageOutput(
            success=False,
            errors=["rtl/top.v:10: syntax error"],
    ...
    def test_lint_fail_max_retries_exceeded_ends(self):
        """Test lint failure after max retries ends pipeline."""
        state = create_initial_state("/tmp/test")
        state["lint_output"] = StageOutput(
            success=False,
            errors=["rtl/top.v:10: syntax error"],
    ...
    def test_lint_no_output_defaults_to_debugger(self):
        """Test missing lint output routes to debugger."""
        state = create_initial_state("/tmp/test")
        # lint_output is None
        state["retry_count"] = {"lint": 0, "sim": 0, "synth": 0}
        state["token_usage"] = 0
